...
LOG_FORMAT (env, default "text"):
  ...
  json  - one JSON object per line: timestamp, level, logger, message,
          ...
          telemetry (trigger events, OCR-submission/latency) and the
          OCR service's per-task decision trace (vote, validation,
          confidence math — see worker.py) both use
          `extra={"fields": ...}` for exactly this reason.
