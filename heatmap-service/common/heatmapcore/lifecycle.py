...
lifecycle.py
--------------------------------------------------------------------
The idle/process state machine shared by the detector and the OCR
service, and the self-healing checkpoint that survives a restart.

NEW in this rewrite — the reference alpr_api.py/video_processor.py had
no such state machine: an Engine process was either running with
whatever cameras were attached to it, or not running at all, and a
container restart depended entirely on startup_reconcile() re-adding
every camera found in `cameras:config`, with no persisted notion of
"was this deployment supposed to be idle or actively processing" or
"how many engines/workers were up".

Four operations, same shape on both services:

    start_idle()     load + warm up whatever this service's "models"
                      are (YOLO engines for the detector, PaddleOCR
                      workers for the OCR service) but do no real work
    ...
    start_process()   begin doing the actual job with whatever is
                      ...
                      OCR service, that means workers start pulling
...
logger = setup_logger("heatmapcore.lifecycle")
