...
tracker.py
--------------------------------------------------------------------
ByteTrack, unmodified from the reference pipeline (eyerik_face_tracker.py).
Kept as its own module purely for a clearer file layout — the tracking
math, thresholds and Kalman filter are untouched so tracking behaviour
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, flag_fdf=0, detbb=None, landmarks=None):
        ...
        self.flag_fdf = flag_fdf  # store the class ID
        self.detbb = np.asarray(detbb, dtype=float) if detbb is not None else None  # store original xyxy detection
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        ...
        self.detbb = np.asarray(new_track.detbb, dtype=float) if new_track.detbb is not None else None  # update detbb
...
class BYTETracker(object):
    ...
    def update(self, output_results, img_info, img_size):
        ...
        # MODIFIED: Handle varying output shapes to include landmarks
        if output_results.shape[1] >= 6:
            ...
            scores = output_results[:, 4]
            bboxes = output_results[:, :4]
            flags = np.zeros_like(scores)  # default: no flag
            landmarks = None
        else:
        ...
        flags_keep = flags[remain_inds]  # keep flags for high-confidence detections

        # EXTRACT landmarks for respective groups
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = []
            # ADDED: zip landmarks_keep and pass them
            for tlbr, score, flag, lm in zip(dets, scores_keep, flags_keep, landmarks_keep):
                tlwh = STrack.tlbr_to_tlwh(tlbr)
                detections.append(STrack(tlwh, score, flag, detbb=tlbr, landmarks=lm))
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            ...
            for det in output_results[inds_second]:
                ...
                lm = det[6:] if det.shape[0] > 6 else None  # EXTRACT landmarks
                detbb = det[:4]  # preserve original xyxy
