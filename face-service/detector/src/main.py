...
via bus.wait_until_available(), same discipline as the heatmap
reference (bind the health port, come up idle, reconcile once Redis
answers).
--------------------------------------------------------------------
"""
...
from facecore.lifecycle import PHASE_STOPPED, ServiceLifecycle
...
def main():
    ...
    logger.info(
        "Starting face detector | device preference=%s | rtsp base=%s | max_cameras_per_engine=%d",
    ...
    def on_demand_changed(is_processing: bool):
        ...
        # the (camera-agnostic) recognizer to mirror it. Fires only from
        # a live activate/deactivate command — see backend_bridge's
