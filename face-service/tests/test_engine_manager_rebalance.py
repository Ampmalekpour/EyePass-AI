...
new consolidation behaviour that does not exist in the reference
pipeline (which only ever grows the engine pool). Reproduces the
scenario from the original request: a 5-per-engine cap, 10 cameras
that end up fragmented 3 + 2 (or worse) across engines after churn,
and rebalance() must consolidate them back down.
...
bin-packing/migration LOGIC in isolation from YOLO/torch/CUDA, which
this sandbox cannot import anyway (see tests/fakes/stub_modules.py).
...
def _make_manager(max_cameras_per_engine=5):
    topology_changes = []
    mgr = EngineManager(
        model_path="/models/best1.pt", imgsz=800, conf=0.25,
        save_output=False, output_dir="/data", class_labels={0: "Head"},
...
class RebalanceTests(unittest.TestCase):
    ...
    def test_fragmented_10_cameras_across_3_engines_consolidates_to_2(self):
        # Reproduces the exact scenario from the request: cap=5, 10
        # cameras fragmented 4 + 3 + 3 across three engines after churn.
        mgr = _make_manager(max_cameras_per_engine=5)
        ...
        # Every camera must still be tracked, on a SURVIVING engine, and
        # no engine may exceed the cap.
        all_cameras = set()
    ...
    def test_migrated_camera_config_is_replayed_on_new_engine(self):
        ...
        _inject_engine(mgr, 1, ["2", "3", "4", "5", "6"])

        mgr.rebalance()

        # Only 6 cameras total / cap 5 => needed = 2 -> already tight,
        # nothing to do. Use a case that DOES need consolidation instead:
        ...
        # Only the MIGRATED camera gets a replayed 'add' onto the
        # surviving engine's control queue — whichever camera was
        # already there (never drained) needed no replay at all.
    ...
    def test_camera_without_cached_config_is_left_in_place_not_lost(self):
        ...
        # Camera 1 could not be migrated (no cached config), so engine 0
        # must be left up rather than torn down with camera 1 unaccounted for.
