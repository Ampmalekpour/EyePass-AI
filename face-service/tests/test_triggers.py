...
no torch, no camera needed; only cv2/numpy, both available in this
sandbox.
...
from triggers import TriggerTrackState, process_track_triggers  # noqa: E402
...
class LineCrossingTests(unittest.TestCase):
    ...
    def test_no_crossing_while_track_too_young(self):
        # MIN_TRACK_AGE_FOR_CROSSING frames are required before a
        # crossing can even be considered.
        events = None
    ...
    def test_crossing_fires_once_when_track_moves_across_the_line(self):
        # Establish the track on the left side long enough to pass the
        # age + confidence gates.
        for _ in range(config.MIN_TRACK_AGE_FOR_CROSSING + 2):
            events = self._feed(1, x=50, y=100)
        self.assertIsNone(events["line_cross"])

        # Now cross to the right side.
        ...
        # Immediately crossing back must be suppressed by the cooldown.
        events = self._feed(1, x=50, y=100)
...
class RoiTests(unittest.TestCase):
    ...
    def _feed(self, track_id, x, y, score=0.9, ts_step=None):
        return process_track_triggers(
    ...
    def test_roi_entry_fires_once_then_exit_on_leaving(self):
        # roi_entry fires on the frame confirmation is first reached, not
        # on every later frame still inside the ROI.
    ...
    def test_stopped_roi_fires_when_stationary_long_enough(self):
        ...
        # Enter and confirm the ROI.
        for _ in range(config.ROI_ENTRY_CONFIRMATION_FRAMES + 1):
            self._feed(1, x=50, y=50)

        # Manually push STOP_MIN_SAMPLES near-identical positions far
        # enough apart in time to satisfy STOP_TIME_SECONDS, at velocity
        # under STOP_VELOCITY_THRESHOLD — this is what "standing still"
        # looks like to the trigger math.
        ...
        # Must not re-fire on the very next frame (stop_reported latch).
        events = self._feed(1, x=50, y=50)
