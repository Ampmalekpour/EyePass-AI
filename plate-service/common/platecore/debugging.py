"""
debugging.py (platecore)
--------------------------------------------------------------------
Shared visual-debug + timing utilities for both plate services. Ported
verbatim from face_service's common/facecore/debugging.py (the pattern
is fully generic, nothing in it is face-specific) — see that module's
own history for why it exists. Follows the rule debug_recorder.py
already established: a debug failure must never touch the pipeline —
every public function swallows its own exceptions, optionally logs
once, and returns None/False rather than raising.
...
What lives here:
  * env_bool/env_int/env_float/env_str — identical semantics to each
    ...
    new debug code in EITHER service (and platecore itself) doesn't
  ...
  * save_montage — arrange N labelled BGR images into one grid JPEG;
    used by the detector's OCR-submission montage and the OCR
    service's decision montage.
...
def timed(logger: logging.Logger, label: str, level: int = logging.DEBUG, **fields):
    """
        with timed(logger, "ocr_task", task_id=task_id):
            ...
    ...
    genuinely expensive debug payloads (full candidate lists, image
...
def prune_dir(dir_path: str, max_files: int, suffixes: Tuple[str, ...] = (),
    ...
    montages) don't grow unbounded either. max_files <= 0 means
    unlimited (never prunes)."""
    if max_files <= 0:
        return
    try:
        files = [
...
# ---------------------------------------------------------------------
# montage rendering — shared by the detector's OCR-submission montage
# and the OCR service's decision montage.
# ---------------------------------------------------------------------
...
def save_montage(
    ...
    per crop / candidate. A None image renders as a visibly distinct
    "no image" placeholder rather than a blank gap, so a missing image
    is never mistaken for a rendering bug.
