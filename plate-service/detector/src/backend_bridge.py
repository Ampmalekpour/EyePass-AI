...
per-camera `ai_status` hash Django already reads, and the bounded
internal-processing-error auto-restart.

Ported from face_service's backend_bridge.py (structure: durable
active_state ledger written before acting, offline-grace timer,
demand-events notification, cmd request/response worker, startup
reconcile) merged with the reference alpr_api.py's own, plate-specific
behavior that face has no equivalent of:

  * `ai_status` writes ("running" / "stopped" / "stopped_by_user" /
    "idle") at the same transition points alpr_api.py used them, so
    Django's existing read side keeps working unchanged.
  * Bounded internal-processing-error auto-restart
    (_attempt_processing_error_restart), ported from alpr_api.py's own
    function of the same name — triggered when an Engine reports
    status="error" on the status queue (a batch-inference exception, a
    transient resource shortage), NOT by a physical camera disconnect
    (that is the offline-grace path below, entirely separate, exactly
    like the reference kept them separate).

One deliberate behavior change from the reference, adopted from face:
alpr_api.py's `_handle_activated` starts the engine unconditionally,
even if camera_stream's own `cameras:details` says the camera is
currently offline (it would then just fail to open the RTSP relay and
sit there erroring). face's DetectorBridge instead waits for the
camera's next online event before attaching in that case. This is
strictly safer and costs nothing — the durable active_state ledger
still marks the camera active either way — so it is adopted here too.

Also NOT ported: the reference's camera-level pause/resume distinction
(_idle_core/_resume_core, used for heartbeat-timeout handling and as
the first half of an error-restart). The new Engine (engine.py) has no
pause_camera/resume_camera — cameras are only ever fully attached
(add_camera) or fully detached (remove_camera), matching face's
simpler model. Heartbeat-timeout handling itself is also not ported as
an auto-stop trigger: face treats heartbeat lateness as informational
only (see status_snapshot()'s "degraded" flag) and never uses it to
tear a camera down on its own — actual stop/restart is driven only by
(a) camera_stream's own online/offline events, through the
offline-grace timer, and (b) an Engine's own status="error" report,
through the bounded auto-restart below. This avoids a second,
overlapping "is this camera actually broken" heuristic racing the
grace timer, and heartbeat lateness is still visible via
status_snapshot() for anyone watching the health endpoint.
...
def rtsp_url(camera_id: str, info: Optional[dict] = None) -> str:
    ...
    (platecore/relay.py) — one path per physical camera, shared by every
    module that uses it."""
    info = info or {}
...
def _bbox_to_points(bbox: Optional[dict]) -> Tuple[float, float, float, float, float, float, float, float]:
    """{x,y,w,h} -> the 4 corner points (x1,y1,x2,y2,x3,y3,x4,y4) the
    engine's stop-ROI polygon expects. Verbatim from alpr_api.py."""
    if not bbox:
...
def build_camera_job(camera_id: str, cfg: dict) -> CameraJob:
    ...
    `plate:cameras:config` (roi: {x,y,w,h}, cross_line: {start:{x,y},
    end:{x,y}}, stop_roi: {x,y,w,h}) — the same shape alpr_api.py's own
    _build_video_config reads. address comes from cameras:details (or
    from cfg itself if the backend already inlines it there)."""
...
class DetectorBridge:
    def __init__(self, bus: RedisBus, engine_manager,
        ...
        # tell the (camera-agnostic) OCR service to do the same — see
        ...
        # ---- internal-processing-error auto-restart (plate-only) ------
    ...
    def _attach(self, camera_id: str, cfg: dict) -> bool:
        ...
        # responsibility for this camera", not "has it produced output".
    ...
    # ================================================================
    # Backend cmd request/response (plate:cmd:ai:request/response)
    # ================================================================
    ...
    def handle_deactivated(self, camera_id: str) -> bool:
        ...
        # Django's own UI distinguishes "we turned it off" from "it fell
        # over on its own", exactly like alpr_api.py's _handle_deactivated.
    ...
    # ================================================================
    # camera_stream events (plate:camera:events — singular, see keys.py)
    # ================================================================
    ...
    # ================================================================
    # Internal-processing-error auto-restart (plate-only; distinct from
    # the offline-grace path above — this is for an Engine's own
    # status="error" report, e.g. a batch-inference exception or a
    # transient resource shortage, not a physical camera disconnect).
    # Ported from alpr_api.py's _attempt_processing_error_restart, with
    # the reference's idle-then-resume replaced by this rewrite's plain
    # detach-then-reattach (see module docstring).
    # ================================================================
    ...
    def handle_engine_error(self, camera_id: str, error: Optional[str]):
        ...
        def _worker():
            ...
            if self._attach(camera_id, cfg):
                ...
                # ai_status="running" already written inside _attach()
            else:
    ...
    # ================================================================
    # Status (informational only for stop/restart purposes — actual
    # stop/restart is driven entirely by the offline-grace path and
    # handle_engine_error() above; see module docstring for why
    # heartbeat lateness itself is not a third trigger)
    # ================================================================
