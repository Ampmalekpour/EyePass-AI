...
assigned to which engine. Ported from face_service's engine_manager.py
(the pattern is fully generic — nothing here is face-specific) with
two behaviours beyond the reference plate EngineManager
(video_processor.py):

  1. It plugs directly into platecore.lifecycle.ServiceLifecycle:
  ...
  2. rebalance(): the reference EngineManager only ever grows (a new
     ...
     more engines than necessary (6-per-engine cap, 10 cameras ->
     2 + 2 + ... after churn). rebalance() detects when the current
     camera count would fit into fewer engines and consolidates: it
     replays each drained camera's last known add_camera() config onto
     a fuller engine, then stops the emptied engine(s). A migrated
...
class EngineManager:
    def __init__(
        ...
        # line. (Discovered from face_service's own engine_manager.py —
        # kept here verbatim since it's a correctness requirement, not a
        # style choice.)
    ...
    # ================================================================
    # platecore.lifecycle.ServiceLifecycle hooks
    # ================================================================
    ...
    def _start_engine_locked(self) -> int:
        ...
        # would leave status_queue on the fork context while the child
        # was spawned.
