...
pool.py (ocr_service)
--------------------------------------------------------------------
`OcrPool` is the OCR service's equivalent of the detector's
EngineManager: it owns a pool of OcrWorker subprocesses and implements
the four lifecycle hooks ServiceLifecycle drives. Ported from
face_service's recognizer/src/pool.py — this pattern is fully generic,
nothing in it is face-specific.

    start_idle(count)   spawn `count` OcrWorker processes, each
                         loading its two PaddleOCR pipelines (car +
                         motorcycle) and warming up, but NOT pulling
                         tasks yet (processing_event starts clear).
                         Waits (bounded by WORKER_LOAD_TIMEOUT_SEC) for
                         them to report ready via their loaded_event.

    stop_idle()          tear every worker in the pool down completely
                         (signals stop_event, joins, terminates
                         stragglers). Fully releases the loaded models.

    start_process()      set the shared processing_event — every
                         worker in the pool starts pulling
                         `plate:internal:ocr:tasks`. Cheap: no process
                         restart, models stay warm.

    stop_process()        clear processing_event — workers stop
                         pulling new tasks but stay loaded and warm.
...
logger = setup_logger("ocr_service.pool")
...
class OcrPool:
    ...
    def _start_watchdog_locked(self):
        ...
        self._watchdog_thread = threading.Thread(target=self._watchdog_loop, daemon=True, name="ocr-watchdog")
